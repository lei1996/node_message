...
const { SealUserTimeout, SealIpTimeout } = require("../utils/const");
 ...
 * @param ctx Context
 */
async function searchExpression(ctx) {
  const { keywords } = ctx.data;
  if (keywords === "") {
    return [];
  }
...
async function sealUser(ctx) {
  ...
  assert(
    !existMemoryData(MemoryDataStorageKey.SealUserList, userId),
    "用户已在封禁名单"
  );
 ...
 */
async function sealUserOnlineIp(ctx) {
